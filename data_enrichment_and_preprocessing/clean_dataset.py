"""
파일명: clean_dataset.py
설명: 데이터셋 정제 및 전처리 작업 수행
단계 및 처리과정:
1. 원본 데이터 로드: 초기 수집된 Raw 데이터셋을 메모리에 로드합니다.
2. 결측치 및 이상치 처리: 비어있거나 논리적으로 맞지 않는 이상값을 식별하고 보간/제거합니다.
3. 데이터 타입 변환: 문자열, 날짜, 숫자형 데이터를 모델링에 적합한 데이터 타입으로 변환합니다.
4. 피처 스케일링 준비: 연속형 변수와 범주형 변수의 스케일링 전처리를 수행합니다.
5. 정제 데이터 저장: 정제 과정이 완료된 깔끔한(Clean) 데이터셋을 파일로 저장합니다.
"""
