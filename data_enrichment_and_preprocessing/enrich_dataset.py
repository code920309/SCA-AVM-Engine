"""
파일명: enrich_dataset.py
설명: 데이터셋에 외부 피처 및 파생 변수를 추가하여 강화(Enrichment)
단계 및 처리과정:
1. 정제 데이터 로드: 앞선 파이프라인에서 정제된 데이터셋을 로드합니다.
2. 외부 피처 조인: 교통, 학군, 상권 등의 외부 특성 데이터를 결합합니다.
3. 파생 변 생성: 기존 변수의 조합 또는 비율을 통해 새로운 변수를 도출합니다.
4. 변수 검증: 새롭게 생성된 피처의 중요도 및 유효성을 일차 검증합니다.
5. 강화 데이터 저장: 정보가 강화된(Enriched) 데이터셋을 후속 처리를 위해 저장합니다.
"""
