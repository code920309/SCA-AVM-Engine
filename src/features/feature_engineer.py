"""
파일명: feature_engineer.py
설명: 모델 성능 향상을 위한 피처 엔지니어링 및 파생 변수 생성
단계 및 처리과정:
1. 기초 피처 상관성 분석: 피처들 간의 기초적인 통계적 상관관계를 분석합니다.
2. 파생 교차항 생성: 비선형 관계 포착을 위해 피처 간 교차항을 생성합니다.
3. 도메인 변수 개발: LTV, 평당 단가 등 부동산 도메인 특화 변수를 도출합니다.
4. 타겟 인코딩 적용: 범주형 데이터에 대해 리키지를 방지하며 타겟 인코딩을 수행합니다.
5. 최적 피처 셀렉션: 중요도 기반으로 유의미한 훈련용 피처 풀을 구성합니다.
"""
