"""
파일명: address_restorer.py
설명: 결측되거나 오류가 있는 주소 데이터 복원 및 정제
단계 및 처리과정:
1. 불량 주소 식별: 규칙에 어긋난 주소 컬럼 패턴을 스캔하여 식별합니다.
2. 분할 및 토큰화: 시/도/구/동/지번 정보를 각각 분리하여 토큰으로 나눕니다.
3. 오타 교정 및 보간: 마스터 사전과 문자열 유사도를 비교하여 오타를 교정합니다.
4. 누락 구역 보완: 좌표 기반 리버스 지오코딩으로 누락된 행정 구역명을 채웁니다.
5. 정제 주소 출력: 표준화 양식으로 복원된 주소를 기존 데이터에 업데이트합니다.
"""
