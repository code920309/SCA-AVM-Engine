...
설명: 데이터 수집기 구현을 위한 기본 베이스 클래스 및 모듈
단계 및 처리과정:
1. 공통 인터페이스 설계: 모든 하위 데이터 컬렉터가 상속받아야 할 구조를 정의합니다.
2. 세션/인증 관리: 외부 소스에 대한 공통 HTTP 세션 및 API 인증 토큰을 관리합니다.
3. 재시도 로직 구현: 통신 장애 발생 시의 백오프 및 재시도 메커니즘을 구비합니다.
4. 기본 로깅 설정: 수집 상태 및 진행률에 대한 표준 로깅 형식을 지정합니다.
5. 추상 메서드 제공: 특화된 데이터 파싱 로직을 위한 추상 메서드를 정의합니다.
"""
