"""
파일명: initial_national_collector.py
설명: 전국 단위의 초기 데이터 수집 로직
단계 및 처리과정:
1. 전국 범위 설정: 전체 법정동 마스터 코드를 순회할 기초 설정을 구성합니다.
2. 병렬 수집 스케줄링: 대용량 데이터의 수집을 위해 스케줄링을 수행합니다.
3. 원시 데이터 덤프: 각 지역별 데이터를 Raw 형식으로 보관합니다.
4. 청크 단위 병합: 데이터를 청크별로 임시 파일로 병합하여 메모리를 관리합니다.
5. 수집 완료 검증: 누락된 지역이 없는지 검수하고 마스터 데이터를 확정합니다.
"""
