"""
파일명: create_base_dataset.py
설명: 모델 학습을 위한 기본(Base) 데이터셋 생성
단계 및 처리과정:
1. 원시 수집 데이터 스캔: 덤프된 기초 데이터를 모두 불러옵니다.
2. 데이터 스키마 통일: 컬럼의 네이밍과 데이터 형식을 스키마에 맞게 통일합니다.
3. 키-밸류 테이블 병합: 타겟 변수와 설명 변수를 공통 인덱스 기준으로 Join합니다.
4. 중복 및 노이즈 일차 제거: 명백한 중복 데이터와 노이즈를 필터링합니다.
5. 베이스 셋 파일화: 파이프라인 후속 단계에 전달될 Base 데이터셋을 출력합니다.
"""
